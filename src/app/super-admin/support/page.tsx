  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast({
          ...
          description: "Could not load support ticket data.",
          variant: "destructive"
      });
    } finally {
      setIsLoading(false);
    ...
    return tickets.filter(ticket => {
      ...
      const matchesSearch = searchLower 
        ? ticket.requester.email.toLowerCase().includes(searchLower) ||
          ...
          ticket.requestDetails.requestedPlan.toLowerCase().includes(searchLower)
        : true;
            ...
            {isLoading ? (
              ...
              filteredTickets.map((ticket) => (
                <TableRow key={ticket.id}>
                  <TableCell>
                    <p className="font-medium">Plan change to <span className="capitalize">{ticket.requestDetails.requestedPlan}</span></p>
                    <p className="text-sm text-muted-foreground">
                        From: <span className="capitalize">{ticket.requestDetails.currentPlan}</span> | Price: {ticket.requestDetails.price}
                    </p>
                  </TableCell>
                  <TableCell>
                   ...
                   <TableCell>
                     ...
                     <p className="text-sm text-muted-foreground font-mono">{ticket.organization.id}</p>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>{format(ticket.createdAt.toDate(), 'MMM d, yyyy - p')}</TableCell>
                </TableRow>
              ))
            ) : (
              <TableRow>
